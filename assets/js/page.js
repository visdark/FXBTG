...
$(function () {
    ...
    var _ajaxOption_Submit = {
        ...
        complete: function () {
            $("#submit_main").text("注册").removeAttr('disabled');
        },
        statusCode: {
            200: function (data) {
                if (data.Message == "OK") {
                    ...
                    if (iUserID > 0) {
                        $.globalMessenger().post({ message: "注册成功", hideAfter: 2, type: 'success', showCloseButton: true });
                    }
                    else {
                        $.globalMessenger().post({ message: "您已注册为会员，正在为您跳转到在线办公室。", hideAfter: 2, type: 'success', showCloseButton: true });
                    }
                    //定时跳转
                    setTimeout(function () {
    ...
    function reloadForm() {
        ...
        $("#submit_main").text("注册FXBTG").removeAttr('disabled');
    }
    ...
    function submit_Main() {
        var $tel_main = $('#tel_main'),
            ...
            _tel = $tel_main.val(),
            _code = $code_main.val(),
            _checkboxWarning = $checkboxWarning.is(":checked"),
            _name = $name.val();
        ...
        if (!_checkboxWarning) {
            strError += "1";
            $.globalMessenger().post({ message: "请阅读并同意FXBTG相关合约", hideAfter: 2, type: 'error', showCloseButton: true });
        }
